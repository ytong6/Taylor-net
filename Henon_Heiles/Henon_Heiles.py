...
def get_args():
    ...
    parser.add_argument('--n_particles', default=1, type=int, help='number of particles')
...
def plot_traj(prediction=None, times=None, true_trajs=None, training_data=None):
    ...
    if prediction is not None:
        ...
        plt.scatter(prediction[:, 2], prediction[:, 3], label='Taylor-net Prediction', c=to_np(times.reshape(-1)))

    if training_data is not None:
        training_data= to_np(training_data)
        plt.plot(training_data[:, 2], training_data[:, 3], c='r',lw=4, label='Training period',zorder=3)

    if true_trajs is not None:
        true_trajs = to_np(true_trajs)

        plt.plot(true_trajs[:, 2], true_trajs[:, 3], lw=1.5,c='C0', label='True trajectory',zorder=2)

    plt.title("Hénon Heiles problem prediction",fontsize=12)
    plt.xlabel('p')
    plt.ylabel('q')
    plt.legend()
    plt.draw()
...
class TpTrained(nn.Module):
    def __init__(self, args):
        ...
        self.nx = args.nx
        self.params_pos = nn.Parameter(torch.randn(self.nx, args.n_particles * args.dimension, args.hidden_dim) *
                                       math.sqrt(2. / args.n_particles * args.dimension / args.hidden_dim / self.nx), requires_grad=True)
        self.params_neg = nn.Parameter(torch.randn(self.nx, args.n_particles * args.dimension, args.hidden_dim) *
                                       math.sqrt(2. / args.n_particles * args.dimension / args.hidden_dim / self.nx), requires_grad=True)
        self.b = nn.Parameter(torch.zeros(1, args.n_particles * args.dimension), requires_grad=True)
        self.k = []
    ...
    def forward(self, x):
        if self.relu:
            ...
            return self.b - torch.matmul(z_neg, self.params_neg.transpose(1, 2)).sum(dim=0) + torch.matmul(z_pos,
...
def gen_data(args,n_samples, data_type='train'):
    ...
    with torch.no_grad():
        for i in range(n_samples):
            ...
            p0 = 1.0 * torch.rand(1, args.n_particles * args.dimension) - 0.5
            q0 = 1.0 * torch.rand(1, args.n_particles * args.dimension) - 0.5
