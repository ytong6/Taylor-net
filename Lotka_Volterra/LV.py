...
def plot_traj(prediction=None, times=None, true_trajs=None, training_data=None):
    ...
    plt.title("Lotka Volterra problem prediction",fontsize=12)
    plt.xlabel('p')
    plt.ylabel('q')
    plt.legend()
    plt.draw()
...
class VqTrained(nn.Module):
    def __init__(self, args):
        ...
        self.nx = 8
        self.params_pos = nn.Parameter(torch.randn(self.nx, d, 128) * math.sqrt(2. / 128. / self.nx), requires_grad=True)
        self.params_neg = nn.Parameter(torch.randn(self.nx, d, 128) * math.sqrt(2. / 128. / self.nx), requires_grad=True)
        self.b = nn.Parameter(torch.zeros(1, d), requires_grad=True)
        self.k = []
        self.relu = args.use_ReLU
        k = 1
...
class Tp(nn.Module):
    def forward(self, p):
        #return p #Pendulum
        return 1. - torch.exp(p)
class Vq(nn.Module):
    def forward(self, q):
        #return torch.sin(q) #Pendulum
        return 2. - torch.exp(q)
